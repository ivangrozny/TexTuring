...
class ViewPort extends GuiElement { 
  ...
  void scroll(int scroll){
    if(src.width/src.height<= 1) zoom = constrain(zoom +0.05*scroll, 0.1, src.height/coords.size.y);  // src image = paysage
    if(src.width/src.height > 1) zoom = constrain(zoom +0.05*scroll, 0.1, src.width/coords.size.x);  // src image = portrait
  ...
  void update(){
    ...
    if( viewing ){    
      viewing = false ;
      // set renderZone size
      if(lastRenderTime <0.05) { renderZone.size.x+=10 ;} else if (lastRenderTime >0.08) { renderZone.size.x-=10 ;};
      if(lastRenderTime <0.05) { renderZone.size.y+=10 ;} else if (lastRenderTime >0.08) { renderZone.size.y-=10 ;};
      ...
      srcMin = render(srcMin, int( renderZone.size.x/zoom ) );
    }
